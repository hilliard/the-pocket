...
export const POST: APIRoute = async ({ request, cookies, redirect }) => {
  try {
    ...
    if (type === 'album') {
      await query(
        'UPDATE albums SET title = $1, price_cents = $2, genre = $3, cover_url = $4 WHERE id = $5 AND artist_human_id = $6',
        [title, price_cents, genre, cover_url, id, session.humanId]
      );
    } else if (type === 'song') {
      await query(
        'UPDATE songs SET title = $1, individual_price_cents = $2, genre = $3 WHERE id = $4 AND artist_human_id = $5',
        [title, price_cents, genre, id, session.humanId]
      );
    } else if (type === 'merch') {
      await query(
        'UPDATE merch SET title = $1, price_cents = $2, image_url = $3 WHERE id = $4 AND artist_human_id = $5',
        [title, price_cents, cover_url, id, session.humanId]
      );
    } else {
